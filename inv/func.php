...
if (isset($_POST['barangmasuk'])) {
    $barcodenya = $_POST['barcodenya'];
    $tambahstok = $_POST['tambahstok'];

    $cekstoksekarang = mysqli_query($conn, "select * from stok where barcode='$barcodenya'");
    $ambildatanya = mysqli_fetch_array($cekstoksekarang);

    $stoksekarang = $ambildatanya['stok'];
    $tambahkanstok = $stoksekarang + $tambahstok;

    $addtomasuk = mysqli_query($conn, "insert into tmasuk (barcode, mjumlah) values('$barcodenya','$tambahstok')");
    $updatestokmasuk = mysqli_query($conn, "update stok set stok='$tambahkanstok' where barcode='$barcodenya' ");
    if ($addtomasuk && $updatestokmasuk) {
        header('location:bmasuk.php');
    } else {
        echo 'Gagal';
...
if (isset($_POST['produkkeluar'])) {
    ...
    $addtokeluar = mysqli_query($conn, "insert into tkeluar (barcode, kjumlah) values('$barcodenya','$keluarstok')");
    $updatestokkeluar = mysqli_query($conn, "update stok set stok='$keluarkanstok' where barcode='$barcodenya' ");
    if ($addtokeluar && $updatestokkeluar) {
        header('location:bkeluar.php');
    } else {
        echo 'Gagal';
