...
<body class="">
    <div class="flex min-h-screen flex-row bg-gray-100 text-gray-800 overflow-y-auto">
        ...
        <main class="fixed lg:left-[250px] mb-6 max-h-screen overflow-y-auto">
            <div class="p-8 ml-10 mt-6 mr-10 mb-10 rounded shadow bg-white">
                <div>
                    <p class="mb-2 text-[32px] font-bold ">Data Barang Keluar</p>
                </div>
                    ...
                    <table id="example" class="stripe hover table-fixed" style="width:100%; padding-top: 1em;  padding-bottom: 1em;">
                        <thead>
                            <tr>
                                ...
                                <th data-priority="5">Tanggal Keluar</th>
                                <th data-priority="6" class="max-w-[140px]">Jumlah Barang Keluar</th>
                                <th data-priority="7" class="max-w-[110px]">Aksi</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php
                            $ambilsemuadatastok = mysqli_query($conn, "select * from tkeluar k, stok s where s.idbarang = k.idbarang");
                            while ($data = mysqli_fetch_array($ambilsemuadatastok)) {
                                $barcode = $data['barcode'];
                                $nbarang = $data['nbarang'];
                                $jenis = $data['jenis'];
                                $letak = $data['letak'];
                                $ktanggal = $data['ktanggal'];
                                $kjumlah = $data['kjumlah'];
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td class="text-center"><?= $ktanggal ?></td>
                                    <td class="text-center"><?= $kjumlah ?></td>
                                    <td class="text-center">
                                        <button data-modal-target="hapuskeluar" data-modal-toggle="hapuskeluar" data-target="#hapuskeluar" data-idkeluar="<?php echo $idkeluar ?>" data-id="<?php echo $idbarang ?>" data-jumlah="<?php echo $kjumlah ?>" data-name="<?php echo $nbarang ?>" class="btnhapusmasuk bg-red-600 hover:bg-red-300 text-white font-semibold hover:text-red-800 py-2 px-4 mr-2 border border-red-600 hover:border-transparent rounded " type="button">Hapus</button>
                ...
                <div class="">
                    <button data-modal-target="tambahmodal" data-modal-toggle="tambahmodal" class="bg-blue-600 hover:bg-blue-300 text-white font-semibold hover:text-blue-800 py-2 px-4 mr-2 border border-blue-600 hover:border-transparent rounded " type="button">Tambah Barang Keluar</button>
                    <a class="bg-blue-600 hover:bg-blue-300 text-white font-semibold hover:text-blue-800 py-2 px-4 mr-2 border border-blue-600 hover:border-transparent rounded " href="printkeluar.php" target="_blank">Print</a>
                </div>
    ...
    <script>
        ...
        $(".btnhapusmasuk").on("click", function() {
            ...
            var idkeluar = $(this).attr('data-idkeluar');
            var nbarang = $(this).attr('data-name');
            var kjumlah = $(this).attr('data-jumlah');
            console.log(idkeluar)
            $('input[name="idbaranghapuskeluar"]').val(idbarang);
            $('input[name="jumlahbaranghapuskeluar"]').val(kjumlah);
            $('input[name="idkeluarhapus"]').html(idkeluar);
            $('#namabaranghapuskeluar').html(nbarang);
            ...
            <form class="p-4 md:p-5" method="post">
                <div class="grid gap-4 mb-4 grid-cols-2">
                    ...
                    <div class="col-span-2 sm:col-span-1">
                        ...
                        <input type="number" name="keluarstok" id="keluarstok" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Jumlah Barang" required="">
                    </div>
                </div>
                <button type="submit" name="produkkeluar" class="text-white inline-flex items-center bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
...
<!-- modal hapus-->
<div id="hapuskeluar" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-md max-h-full">
        <!-- Modal content -->
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            <!-- Modal header -->
            <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                ...
                <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="hapuskeluar">
                    <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                    </svg>
                    <span class="sr-only">Close modal</span>
            ...
            <form class="p-4 md:p-5" method="post">
                <div class="grid gap-4 mb-4 grid-cols-2">
                    <div class="col-span-2">
                        <label for="barcode" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Apakah Anda Yakin Ingin Menghapus Produk "<label id="namabaranghapuskeluar"></label>" ?</label>
                        <input type="hidden" name="idbaranghapuskeluar">
                        <input type="hidden" name="jumlahbaranghapuskeluar">
                        <input type="hidden" name="idkeluarhapus">
                    </div>
                    <div>
                        <button type="submit" name="hapusbarangkeluar" class="text-white inline-flex items-center bg-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800">
                            Hapus Barang
                        </button>
                    </div>
            </form>
